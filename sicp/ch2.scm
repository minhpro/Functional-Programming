...
;: (last-pair list)-> return the list of last element
; + list is empty(nil), then return nil
; + (cdr list) is nil, then return list
; + otherwise, return (last-pair (cdr list))
...
;; EXERCISE 2.18
;: (reverse (list 1 4 9 16 25)) = (reverse-iter items nil)
; (reverse-iter items answer)
; if items is nil return answer
...
;: (reverse x)
;: (deep-reverse x)
;; (deep-reverse-iter x answer)
; x is empty, return answer
; y = (car x), 
; if y is a pair, call deep-reverse-iter with
;     x = (cdr x)
;     answer = (cons a answer) with a = deep-reverse (car x)
; else y is not a pair, deep-reverse-iter (cdr x) (cons (car x) answer)                
   
(define nil ())

(define (deep-reverse-iter items answer)
  (if (null? items)
      answer
      (let ((y (car items))
            (x (cdr items)))
        (if (pair? y)
            (deep-reverse-iter x (cons (deep-reverse y) answer))
            (deep-reverse-iter x (cons y answer))))))
